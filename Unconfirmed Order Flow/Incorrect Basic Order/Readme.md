...
The order will be created as **confirmed** not unconfirmed because the request sends `paid = 0` rather than `confirmed = 0`

## Request flow

1. Add products to the cart
2. Create a reservation
4. Create a **confirmed** order using the reservation token from the previous step

(Take the payment)

5. Confirm the order - this request will return HTTP 409 because the order is already confirmed
6. Save the payment
