...
An example of creating an unconfirmed order with a membership passing in a member name and email address. This is the most basic scenario for a membership order.
...
4. Create an unconfirmed order using the reservation token from the previous step
5. Confirm the order

## Notes

The membership created as part of the unconfirmed order is `Inactive` - the membership is made `Active` when the order is confirmed.
